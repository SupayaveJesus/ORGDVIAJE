package com.example.orgdeviaje.viewmodels
    ...
    class LugarViewModel : ViewModel() {
        ...
        fun updatePlace(id: Int, lugar: Lugar, onResult: (Boolean) -> Unit) {
            viewModelScope.launch {
                try {
                    val actualizado = RetrofitInstance.api.editarLugar(id, lugar)
                    val index = listLugares.indexOfFirst { it.id == id }
                    if (index >= 0) {
                        listLugares[index] = actualizado
                    }
                    onResult(true)
                } catch (e: Exception) {
